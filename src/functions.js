...
bar = function (doubleArray) {
  for (var index = 0; index < doubleArray.length; index++) {  // Loops through array
    if (typeof doubleArray[index] == "number")  // Checks that array element is number
      doubleArray[index] = 2 * doubleArray[index];  // Doubles the number
    else
      return false;  // Exits function call if array element is not number.
  }
...
function parseGit(logArray) {
  ...
  for (var index = 0; index < logArray.length; index++) {  // Loops over logArray
    tempString = logArray[index];  // Holds current log input.

    var index2 = 0;
    hash = "";

    while (tempString[index2] != " ") {  // Pulls hash from input string.
      hash = hash + tempString[index2];
      index2++;
    }
    ...
    while (tempString[index2 + 1] != "\"") {  // Pulls date from input string.
      date = date + tempString[index2];
      index2++;
    }
    ...
    while (index2 < tempString.length - 1) {  // Pulls message from input string.
      message = message + tempString[index2];
      index2++;
    }

    gitLogArray.push(new GitLog(hash, new Date(date), message));  // Pushes new instance of GitLog to gitLogArray
  }
